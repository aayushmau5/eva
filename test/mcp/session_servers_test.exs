defmodule Eva.MCP.SessionServersTest do
  use ExUnit.Case, async: false

  alias Eva.Agent.Tools
  alias Eva.Coding.Resources
  alias Eva.MCP.{Config, Events, SessionServers, ToolAdapter}
  ...
  defp build(attrs) do
    struct(SessionServers, Keyword.merge([servers: [], overrides: %{}, snapshots: %{}], attrs))
  end
  ...
  describe "new/2" do
    test "returns a snapshot for a server it started" do
      ...
      mcp = SessionServers.new(resources)
    ...
    test "a replayed override beats the config file" do
      ...
      mcp = SessionServers.new(resources, %{"srv" => false})
    ...
    test "survives a server whose command does not exist" do
      ...
      assert %{"bad" => _snapshot} = SessionServers.new(resources).snapshots
    end
  end

  describe "tools/1" do
    test "returns nothing without servers or clients" do
      assert SessionServers.tools(build(servers: [])) == []
      assert SessionServers.tools(build(servers: [build_config()])) == []
    end
    ...
    test "adapts the tools of a running client" do
      ...
      assert [%Tools.AgentTool{} = tool] = SessionServers.tools(build(servers: [config]))
    ...
    test "collects across servers, skipping ones with no client" do
      ...
      assert [tool] = SessionServers.tools(build(servers: [dead, alive]))
    ...
    test "skips a server disabled in config even while its client runs" do
      ...
      assert SessionServers.tools(build(servers: [config])) == []
    end
    ...
    test "skips a server disabled by a session override" do
      ...
      assert SessionServers.tools(mcp) == []
    end
    ...
    test "an override re-enables a server disabled in config" do
      ...
      assert [tool] = SessionServers.tools(mcp)
  ...
  describe "list/1" do
    test "reports enabled and disabled servers side by side" do
      ...
      assert [on_info, off_info] = SessionServers.list(mcp)
    ...
    test "reports an enabled server with no snapshot as failed" do
      assert [%{status: :failed}] = SessionServers.list(build(servers: [build_config()]))
    end
  end
  ...
  describe "set_enabled/4 - :session" do
    test "disables a server, records the override, and hides its tools" do
      ...
      assert {:ok, mcp} = SessionServers.set_enabled(mcp, config.name, false, :session)
    ...
    test "re-enabling restores the snapshot and the tools" do
      ...
      assert {:ok, mcp} =
               SessionServers.set_enabled(build(servers: [config]), config.name, true, :session)

      assert mcp.snapshots[config.name].status == :connected
      assert length(SessionServers.tools(mcp)) == 1
    end

    test "leaves the config file alone" do
      resources = write_mcp_json(mktmp(), %{"srv" => %{"type" => "stdio", "command" => "echo"}})
      mcp = SessionServers.new(resources)

      assert {:ok, _mcp} = SessionServers.set_enabled(mcp, "srv", false, :session)

      assert {[%Config{enabled: true}], []} = Config.parse(resources)
    end

    test "returns :not_found for an unknown server" do
      assert SessionServers.set_enabled(build(servers: []), "nope", false, :session) ==
               {:error, :not_found}
    end
  end
  ...
  describe "set_enabled/4 - :persist" do
    setup do
      resources = write_mcp_json(mktmp(), %{"ctx7" => %{"type" => "stdio", "command" => "echo"}})
      %{resources: resources, mcp: SessionServers.new(resources)}
    end

    test "writes enabled: false back to the file it came from", %{mcp: mcp, resources: resources} do
      assert {:ok, mcp} = SessionServers.set_enabled(mcp, "ctx7", false, :persist)

      assert {[%Config{enabled: false}], []} = Config.parse(resources)
      # the in-memory config is swapped too, so the next prompt agrees with the file
      assert [%Config{enabled: false}] = mcp.servers
      assert [%{status: :disabled}] = SessionServers.list(mcp)
    end

    # A leftover override would silently outrank the file the user just wrote.
    test "clears any session override for that server", %{mcp: %SessionServers{} = mcp} do
      mcp = %SessionServers{mcp | overrides: %{"ctx7" => false}}

      assert {:ok, mcp} = SessionServers.set_enabled(mcp, "ctx7", true, :persist)

      assert mcp.overrides == %{}
      assert [%{session_enabled: nil}] = SessionServers.list(mcp)
    end

    test "reports a write failure without changing state", %{mcp: %SessionServers{} = mcp} do
      absent = build_config(name: "absent-from-file")
      mcp = %SessionServers{mcp | servers: mcp.servers ++ [absent]}

      assert SessionServers.set_enabled(mcp, "absent-from-file", false, :persist) ==
               {:error, :not_found}
    end
  end
  ...
  describe "apply_event/2" do
    test "ServerConnected fills in status and version, leaving other keys alone" do
      ...
      mcp = SessionServers.apply_event(mcp, event)
    ...
    test "ToolsDiscovered replaces the tool list" do
      ...
      mcp =
        SessionServers.apply_event(mcp, %Events.ToolsDiscovered{
          server_name: "srv",
          scope_dir: :global,
          tools: tools
        })
    ...
    test "ServerDisconnected mirrors the client's :failed status" do
      ...
      mcp =
        SessionServers.apply_event(mcp, %Events.ServerDisconnected{
          server_name: "srv",
          scope_dir: :global,
          reason: :process_exit
        })
    ...
    test "ServerError leaves status alone" do
      ...
      mcp =
        SessionServers.apply_event(mcp, %Events.ServerError{
          server_name: "srv",
          scope_dir: :global,
          error: "boom",
          phase: :request
    ...
    test "ignores events for servers this session is not tracking" do
      ...
      mcp =
        SessionServers.apply_event(mcp, %Events.ServerConnected{
          server_name: "ghost",
          scope_dir: :global
        })
    ...
    test "every event module is handled without crashing" do
      ...
      for mod <- Events.modules() do
        assert %SessionServers{} =
                 SessionServers.apply_event(mcp, struct(mod, server_name: "srv"))
      end
    end
  end
end
