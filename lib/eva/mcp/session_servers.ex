defmodule Eva.MCP.SessionServers do
  @moduledoc """
  ...
  alias Eva.MCP.{Client, Config, Events, Supervisor, ToolAdapter}
  ...
  """
  @spec new(Resources.t(), %{String.t() => boolean()}) :: t()
  def new(%Resources{} = resources, overrides \\ %{}) do
    # TODO: bubble up diagnostics as a separate entity
    {servers, _diagnostics} = Config.parse(resources)
  ...
  defp apply_scope(%__MODULE__{} = mcp, config, enabled?, :persist) do
    case Config.set_enabled(mcp.resources, config, enabled?) do
      {:ok, updated} ->
        mcp = %__MODULE__{
          mcp
