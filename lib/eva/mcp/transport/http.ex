defmodule Eva.MCP.Transport.Http do
  @moduledoc """
...
defimpl Eva.MCP.Transport, for: Eva.MCP.Transport.Http do
  require Logger

  alias Eva.MCP.{Config, Protocol, SSE}
  ...
  def send_message(%Http{} = transport, message) do
    ...
    case Task.Supervisor.start_child(Eva.TaskSupervisor, fn ->
  ...
  def close(%Http{listening_pid: pid}) do
    Task.Supervisor.terminate_child(Eva.TaskSupervisor, pid)
    :ok
  end
