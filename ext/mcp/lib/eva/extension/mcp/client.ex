defmodule Eva.Extension.MCP.Client do
  use GenServer
  use TypedStruct

  require Logger

  alias Eva.Extension.MCP.{Config, Events, Protocol, Transport, Transports}

  @registry Eva.Extension.MCP.Registry
  ...
  def whereis(%Config{scope_dir: scope_dir, name: name} = _config) do
    ...
    # The registry is gone, which happens exactly once: while this application is shutting
    # down and `terminate/2` is unsubscribing. Nothing is running if the registry is not,
    # so "no client" is the honest answer — and raising here would abandon the rest of the
    # cleanup for no gain.
    ArgumentError -> nil
  end
