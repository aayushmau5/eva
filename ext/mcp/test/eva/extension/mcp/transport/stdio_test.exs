defmodule Eva.Extension.MCP.Transport.StdioTest do
  ...
  describe "handle_message/2 :stdout" do
    test "returns frames with split lines and updated buffer" do
      ...
      assert {:frames, ["hello"], updated} =
               Eva.Extension.MCP.Transport.handle_message(transport, {:stdout, 1, "hello\n"})
    ...
    test "accumulates partial data in buffer" do
      ...
      assert {:frames, ["hello"], updated} =
               Eva.Extension.MCP.Transport.handle_message(transport, {:stdout, 1, "lo\n"})
  ...
  describe "handle_message/2 :stderr" do
    test "returns log with split lines and updated stderr buffer" do
      ...
      assert {:log, ["error line"], updated} =
               Eva.Extension.MCP.Transport.handle_message(transport, {:stderr, 1, "error line\n"})
    ...
    test "accumulates partial stderr data" do
      ...
      assert {:log, ["error"], updated} =
               Eva.Extension.MCP.Transport.handle_message(transport, {:stderr, 1, "or\n"})
  ...
  describe "handle_message/2 :DOWN" do
    test "returns closed with reason on matching pids" do
      ...
      assert {:closed, :normal} =
               Eva.Extension.MCP.Transport.handle_message(
                 transport,
  ...
  describe "handle_message/2 filtering" do
    test "ignores stdout from wrong os_pid" do
      ...
      assert :ignore =
               Eva.Extension.MCP.Transport.handle_message(transport, {:stdout, 99, "data\n"})
    end
    ...
    test "ignores stderr from wrong os_pid" do
      ...
      assert :ignore =
               Eva.Extension.MCP.Transport.handle_message(transport, {:stderr, 99, "data\n"})
    end
    ...
    test "ignores unknown message types" do
      ...
      assert :ignore = Eva.Extension.MCP.Transport.handle_message(transport, {:exit, 1, 0})
      assert :ignore = Eva.Extension.MCP.Transport.handle_message(transport, :garbage)
    end
  end
  ...
  describe "send_message/2" do
    test "returns error when not connected" do
      assert {:error, :not_connected} =
               Eva.Extension.MCP.Transport.send_message(%Stdio{os_pid: nil}, "irrelevant")
    end
  end
  ...
  describe "close/1" do
    test "returns :ok when already closed" do
      assert :ok = Eva.Extension.MCP.Transport.close(%Stdio{os_pid: nil})
    end
  end
  ...
  describe "connect/1 with echo process" do
    test "connects and returns a transport struct" do
      ...
      Eva.Extension.MCP.Transport.close(transport)
    end
    ...
    test "send_message writes to child stdin, stdout arrives as message" do
      ...
      try do
        Eva.Extension.MCP.Transport.send_message(transport, msg)

        assert_receive {:stdout, ^os_pid, data}, 2000

        {:frames, lines, _t} =
          Eva.Extension.MCP.Transport.handle_message(transport, {:stdout, os_pid, data})
    ...
    test "stderr output surfaces as log lines" do
      ...
      try do
        Eva.Extension.MCP.Transport.send_message(transport, "hello")
    ...
    test "close shuts down the process" do
      ...
      Eva.Extension.MCP.Transport.send_message(transport, "ping")
    ...
    test "handles multiple messages across the connection" do
      ...
      try do
        Enum.reduce(1..3, transport, fn i, t ->
          msg = "msg#{i}"
          Eva.Extension.MCP.Transport.send_message(t, msg)

          assert_receive {:stdout, ^os_pid, data}, 2000

          {:frames, lines, updated} =
            Eva.Extension.MCP.Transport.handle_message(t, {:stdout, os_pid, data})
